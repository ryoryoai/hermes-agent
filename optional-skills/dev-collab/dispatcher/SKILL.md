...
gh pr list -R ryoryoai/hermes-agent --state open --json number,title,headRefName,statusCheckRollup,labels,author
```

PR本文が必要な場合（対応Issueの `Closes #N` 特定等）は、`author.login` が `ryoryoai` であることを確認した**後に**、対象PRのみ `gh pr view <PR> -R ryoryoai/hermes-agent --json body` で取得する。外部作者のPRの本文・diff・コメントは一切取得しない。
...
nohup ~/.local/bin/hermes chat -m gpt-5.5 --provider openai-codex -q "あなたは開発ワーカー。まずread_fileで /Users/ryohei/.hermes/skills/dev-collab-worker/SKILL.md を読み、その規約に従って ryoryoai/hermes-agent の Issue #<N> を解決せよ。" >> ~/agent-workspace/logs/worker-issue<N>.log 2>&1 &
```
...
nohup ~/.local/bin/hermes chat -m gpt-5.5 --provider openai-codex -q "あなたはレビューエージェント。まずread_fileで /Users/ryohei/.hermes/skills/dev-collab-reviewer/SKILL.md を読み、その手順に従って ryoryoai/hermes-agent の PR #<PR> をレビューせよ。" >> ~/agent-workspace/logs/reviewer-pr<PR>.log 2>&1 &
```
...
nohup ~/.local/bin/hermes chat -m gpt-5.5 --provider openai-codex -q "あなたは開発ワーカー。まずread_fileで /Users/ryohei/.hermes/skills/dev-collab-worker/SKILL.md を読め。ryoryoai/hermes-agent の PR #<PR>（Issue #<N>）の必須チェックが失敗した。失敗内容は gh pr checks <PR> -R ryoryoai/hermes-agent と gh pr view <PR> -R ryoryoai/hermes-agent --comments で自分で確認し、既存worktree ~/agent-workspace/issue-<N> で修正して同じブランチにpushせよ。" >> ~/agent-workspace/logs/worker-issue<N>-retry.log 2>&1 &
```
...
再起動の判断: 前回の起動/差し戻しコメントから**45分未満なら見送る**（ワーカーが実行中の可能性があるため）。**45分以上**経過してもworkerのコメント（PR作成報告 or 断念報告）が無い場合は異常終了とみなし、試行回数を1進めて再起動する。
...
直近にマージされたPR（`gh pr list -R ryoryoai/hermes-agent --state merged --limit 5 --json number,title,author,mergedAt` でmergedAtが直近1時間以内）ごとに処理する。対象は作者が `ryoryoai` のPRのみで、対応Issueの特定に本文が必要な場合は、作者確認の**後に**そのPRだけ `gh pr view <PR> -R ryoryoai/hermes-agent --json body` で取得する（外部作者のPR本文は取得しない）。対応Issue #<N> について:
...
レビュー差し戻しまたは再レビュー完了を通知する場合は、Discordサマリに必ず `指摘N件 → 対応M件/反論K件 → 判定` の1行を含める。

## Pitfalls

- ワーカー/レビュアーの起動は必ず `nohup ... &` で非同期に行い、終了を待たない（cronの非活動タイムアウトに掛かるため）
- 起動コマンドには必ず `-m gpt-5.5 --provider openai-codex` を付ける。CLIデフォルトモデル（anthropic系）はClaude Code等とのOAuthトークン共有により認証が不安定で、レビュアーが無言終了する事故が起きた
- 起動コマンドは必ず **terminalツールで直接** 実行する。execute_code（Pythonサンドボックス）の中から起動してはならない — サンドボックス終了時に子プロセスが回収され、ワーカーが起動報告だけ残して静かに消える（Issue #5で実際に発生した事故）
