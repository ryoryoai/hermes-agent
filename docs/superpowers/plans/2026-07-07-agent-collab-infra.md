...
- 必須ステータスチェックのcontext名は正確に `All required checks pass`, `agent-review`（`All required checks pass` は上流CIのメタチェック名。skipped=success扱い）
...
**Interfaces:**
- Produces: スキル名 `dev-collab-dispatcher` / `dev-collab-worker` / `dev-collab-reviewer`。dispatcherはworker/reviewerを `nohup hermes chat -q "..."` で起動する（プロンプト内でSKILL.mdの絶対パスをread_fileさせる）。reviewerはcommit status `agent-review` をセットする
...
nohup ~/.local/bin/hermes chat -q "あなたは開発ワーカー。まずread_fileで /Users/ryohei/.hermes/skills/dev-collab-worker/SKILL.md を読め。ryoryoai/hermes-agent の PR #<PR>（Issue #<N>）が <失敗理由> で失敗した。既存worktree ~/agent-workspace/issue-<N> で修正し、同じブランチにpushせよ。レビュー指摘は gh pr view <PR> --comments で確認せよ。" >> ~/agent-workspace/logs/worker-issue<N>-retry.log 2>&1 &
```
...
## テスト
このPR自体がゲート（All required checks pass + agent-review + auto-merge）の初回検証となる。"
```
...
Expected: `"state": "MERGED"`。All required checks pass + agent-review 全green後に人手ゼロでマージされたこと
...
Expected: ワーカーのPRが作成され、All required checks pass/agent-review が全green → MERGED。Discord #dev に着手・レビュー・マージの通知が流れている
