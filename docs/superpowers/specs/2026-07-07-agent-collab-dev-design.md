...
- branch protection（main）:
  - PR必須（直push禁止）
  - 必須ステータスチェック: `All required checks pass`（上流CIのメタチェック。skipped=success扱い）、`agent-review`
...
- 実体: 指揮エージェントが `hermes chat -q "<タスク指示>"` で起動するheadless hermesセッション。`optional-skills/dev-collab/worker/SKILL.md` の作業規約に従う
- 作業手順:
...
## ブートストラップ時の変更（2026-07-07追記）

- fork mainはセットアップ時にupstream/main（009b42d00）へ同期した。旧mainではCIの依存ドリフトによりtestジョブが恒常的に失敗していたため
- upstream CIはorchestrator（ci.yml）+メタチェック構成に移行済み。必須ステータスチェックは `All required checks pass` + `agent-review` の2つとする（メタチェックはskipped=success扱いのため、docs-only PRでもゲートが完走する）
- `hermes -q` は誤りで、headless実行の正しい構文は `hermes chat -q`
